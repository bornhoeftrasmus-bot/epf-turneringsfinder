...
const CITY_KEYS = ["City", "AddressCity", "LocationCity", "VenueCity", "ClubCity", "TournamentCity", "PostalCity", "PostCity"];
const CENTER_KEYS = ["LocationName", "ClubName", "VenueName", "FacilityName", "CenterName", "CentreName", "TournamentLocation"];
  ...
  try {
    ...
    for (const event of officialEvents) {
      try {
        ...
        if (!model) throw new Error("TournamentSidebarModel missing");
        if (!firstModelKeys.length) firstModelKeys = Object.keys(model).sort();
        ...
        failed.push({ rankedin_id: String(event.EventId || ""), name: event.EventName || "", error: error.message });
    ...
    if (!hasMore && cleanup) stale = await cleanupStaleFutureRows(today);
    ...
    return res.status(200).json({
      ...
      raw_official_found: rawOfficialEvents.length,
      official_found: officialEvents.length,
      duplicate_events_removed: rawOfficialEvents.length - officialEvents.length,
      saved: rows.length,
      failed: failed.length,
      coverage,
      missing_deadlines: rows.filter((row) => !row.deadline).map((row) => ({ rankedin_id: row.rankedin_id, name: row.name })),
      missing_cities: rows.filter((row) => !row.city).map((row) => ({ rankedin_id: row.rankedin_id, name: row.name })),
      missing_centers: rows.filter((row) => !row.center).map((row) => ({ rankedin_id: row.rankedin_id, name: row.name })),
...
async function fetchCalendarPage(page, take = PAGE_SIZE) {
  ...
  const url = `https://api.rankedin.com/v1/calendar/GetEventsAsync?from=${from}&take=${take}&country=45&sport=5&eventType=0&calendarDateFilter=1&calendarOrganization=1`;
...
async function fetchAllOfficialFutureIds(today) {
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    const events = await fetchCalendarPage(page, PAGE_SIZE);
    for (const event of events) {
      if (event.OrganisationName === DPF_ORGANISATION_NAME && isoDateOnly(event.StartDate) >= today && event.EventId) {
...
async function getInfo(id) {
  return fetchJsonWithRetry(`https://api.rankedin.com/v1/tournament/GetInfoAsync?id=${id}&language=en`, `GetInfoAsync ${id}`);
}
...
async function fetchJsonWithRetry(url, label) {
  ...
  for (let attempt = 1; attempt <= RANKEDIN_RETRIES; attempt += 1) {
    try {
      ...
      if (!Array.isArray(data) && label.includes("calendar")) throw new Error(`${label} returned invalid payload`);
      return data;
    } catch (error) {
      lastError = error;
      if (attempt < RANKEDIN_RETRIES) await sleep(250 * 2 ** (attempt - 1));
    }
  }
  throw lastError;
}
...
async function upsertRows(rows) {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/tournaments?on_conflict=rankedin_id`, {
    method: "POST",
...
async function cleanupStaleFutureRows(today) {
  ...
  if (!officialIds.size) {
    return { removed: 0, skipped: true, reason: "official DPF organisation calendar returned zero future IDs" };
  }
  ...
  if (!response.ok) throw new Error(`Supabase stale read ${response.status}: ${await response.text()}`);
  ...
  for (let index = 0; index < staleIds.length; index += 50) {
    ...
    deleteUrl.searchParams.set("rankedin_id", `in.(${chunk.map((id) => `"${id.replaceAll('"', '\\"')}"`).join(",")})`);
    deleteUrl.searchParams.set("tournament_date", `gte.${today}`);
    const deletion = await fetch(deleteUrl, { method: "DELETE", headers: sourceHeaders() });
    if (!deletion.ok) throw new Error(`Supabase stale delete ${deletion.status}: ${await deletion.text()}`);
  }
...
function findCategories(text = "") {
  ...
  if (/junior|u10|u12|u14|u16|u18|ungdom|drenge|piger/i.test(text)) result.push("Junior");
  return result;
}
...
function normalizeRankedinDateTime(value) {
  ...
  const match = raw.match(/^(\d{4})-(\d{2})-(\d{2})[T\s](\d{1,2}):(\d{2})(?::(\d{2}))?(?:\.\d+)?$/);
...
function copenhagenWallTimeToUtcIso(year, month, day, hour, minute, second = "00") {
  const targetWallClock = Date.UTC(Number(year), Number(month) - 1, Number(day), Number(hour), Number(minute), Number(second));
...
function cleanCity(value = "") {
  const city = clean(value).replace(/^\d{4}\s+/, "").replace(/,?\s*(Danmark|Denmark)$/i, "").trim();
...
function cleanCenter(value = "") {
  ...
  if (!center || /^Dansk Padel Forbund(?:s rangliste)?$/i.test(center)) return "";
  return center;
}
...
function cityFromNames(tournamentName = "", locationName = "") {
  ...
  const cities = ["Esbjerg", "Vejle", "Odense", "Kolding", "Viborg", "Aalborg", "Aarhus", "Hasselager", "Svenstrup J", "Brøndby", "København", "Frederikssund", "Holstebro", "Herning", "Horsens", "Roskilde", "Køge", "Næstved", "Slagelse", "Svendborg", "Sønderborg", "Haderslev", "Aabenraa", "Fredericia", "Middelfart", "Silkeborg", "Randers", "Skive", "Hjørring", "Frederikshavn"];
...
function regionFromKnownCity(city = "") {
  ...
  const map = {
    esbjerg: "Syddanmark", vejle: "Syddanmark", odense: "Syddanmark", kolding: "Syddanmark", fredericia: "Syddanmark", middelfart: "Syddanmark", svendborg: "Syddanmark", sønderborg: "Syddanmark", haderslev: "Syddanmark", aabenraa: "Syddanmark",
    viborg: "Midtjylland", aarhus: "Midtjylland", hasselager: "Midtjylland", silkeborg: "Midtjylland", randers: "Midtjylland", skive: "Midtjylland", holstebro: "Midtjylland", herning: "Midtjylland", horsens: "Midtjylland",
    aalborg: "Nordjylland", "svenstrup j": "Nordjylland", hjørring: "Nordjylland", frederikshavn: "Nordjylland",
    brøndby: "Hovedstaden", københavn: "Hovedstaden", frederikssund: "Hovedstaden",
    roskilde: "Sjælland", køge: "Sjælland", næstved: "Sjælland", slagelse: "Sjælland"
  };
  return map[key] || "";
}
